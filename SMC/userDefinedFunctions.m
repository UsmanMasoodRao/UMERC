...
dtime = diff(controllersOutput.time);
energy = controllersOutput.power(1:end-1,3).*dtime;
accEnergy = cumsum(energy);

plot(controllersOutput.time(1:end-1),accEnergy)
title('Controller Energy')
ylabel('Energy (W/sec)')
xlabel('Time (s)')

save('..\Results\ControlFLC')